...
# --                print('NPgm-NP:',child.text)  # --
                value = child.get_text()
                text_children += [x.strip() for x in value[0].split()]
                #text_children.append(value[0].strip())

                NPcodes += value[1]
                non_matched_txt += text_children
            elif child.label[:2] in ["JJ","DT","NN"]:
                child_value_text = [x.strip() for x in child.get_text().split()]
                #child_value_text = []
                #child_value_text.append(child.get_text().strip())

                text_children += child_value_text
                if child.label[:2] == "NN":
                    non_matched_txt += child_value_text
...
# --                print('NPgm-PP:',m)  # --
                if m[0]:
                    ...
                    text_children += [x.strip() for x in child.get_text().split()]
...
# --                        print('NPgm-1: Mk3')  # --
                        for child in level.parent.children:
                            if isinstance(child,NounPhrase):
                                try:
                                    cgm = child.get_meaning().strip() # see <16.06.10> note above
                                except:
                                    break 
...
# --                print('NPgm-2.0:',roots)
        ...
        if matched_txt:
            matched_txt = [x.strip() for x in matched_txt]
            self.sentence.metadata['nouns'] += [(matched_txt, self.meaning, roots[:len(matched_txt)])]


        self.sentence.metadata['nouns_not_matched'] += list(set(non_matched_txt) - set(matched_txt))

        print ('nouns_not_matched')
        print (self.sentence.metadata['nouns_not_matched'])
...
class Sentence:
    ...
    def __init__(self, parse, text, date):
        ...
        self.verb_analysis = {}
        self.events = []
        self.metadata = {'nouns': [], 'nouns_not_matched': []}
    ...
    def get_events(self,require_dyad = 1):
        ...
        meta = {'nouns' : self.metadata['nouns'], 'nouns_not_matched': self.metadata['nouns_not_matched']}
        """print('GF1',events) # --  
# --        print('GF2',meta) # --  
